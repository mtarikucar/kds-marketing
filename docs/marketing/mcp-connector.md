...
| `jeeta.record_consent` | Honour "stop emailing me": records the consent AND flips the opt-out flag in one transaction | `contacts.write` | WRITE | — | no |
| `jeeta.get_consents` | What a contact has said about being contacted, with dates and sources | `contacts.read` | READ | — | no |
