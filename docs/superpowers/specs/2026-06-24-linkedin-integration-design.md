...
- **`linkedin-api.util.ts`** (new): `linkedinRest(path, {accessToken, method, query, body, version?, headers?})` over `safeFetch`; injects `LinkedIn-Version: <YYYYMM>`, `X-Restli-Protocol-Version: 2.0.0`, `Authorization: Bearer`. The version comes from `LINKEDIN_DEFAULT_API_VERSION` in that file — the single source of truth — overridable per-deploy via the `LINKEDIN_API_VERSION` env var (rendered by `deploy.yml`) and per-call via `version`. **`202406`, which this spec originally named as the default, is retired**: it fails every `/rest` call with `Requested version 20240601 is not active`, which is what took LinkedIn publishing down in production. Never re-pin it; whichever YYYYMM is current must be checked against LinkedIn's published version list before shipping (this repo cannot check it). Flat result `{ok, status, data, error:{message,isAuthError}|null}` (mirror `meta-graph.util`). Single **`isLinkedinAuthError(status, body)`** classifier (HTTP 401, `serviceErrorCode`/`code` token errors, `REVOKED_ACCESS_TOKEN`) reused by publish + ads + engagement.
