...
| 4 | Unit/e2e coverage for `platform` admin + `internal-research` + `outbox` worker | Testability, Reliability | M | Closes the per-module test gaps above. |
...
| 8 | Global exception filter emitting a uniform error envelope `{ error, statusCode, requestId, path, timestamp }` | Reliability, Observability | S | Must preserve the existing `/api/internal/*` envelopes — additive only. |
