      ...
      return {
        ...
        message:
          res.reason === 'not-in-mcp-mode'
            ? 'This workspace still has the PLATFORM draining its research queue: researchExecution is SERVER, or it is AUTO with no MCP traffic recorded in the last 14 days. Either way there is nothing for you to lease right now. A workspace OWNER can switch it outright under Settings > Claude connector (the "Who runs the nightly research" card); on AUTO it resolves itself once tool calls from this workspace are being made regularly. Stop polling for now.'
