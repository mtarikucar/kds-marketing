...
export function registerAiLaneTools(registry: McpToolRegistry, deps: AiLaneToolDeps): void {
  registry.register({
    ...
    description:
      'Take the next customer message this workspace is waiting on an answer for, and hold it while ' +
      'you write one. Returns the conversation id and how long it has been queued, or nothing when ' +
      'the queue is empty. THIS IS A POLL: the server cannot wake you, so a workspace that relies on ' +
      'its own Claude needs a scheduled task calling this, and one that does not have one will simply ' +
      'accumulate unanswered customers. Compose the reply with jeeta.read_conversation and ' +
      'jeeta.get_agent (its persona and guardrails are what the customer expects to hear), send it ' +
      'with jeeta.send_message, then close the job with jeeta.complete_reply_job — a lease you never ' +
      'close returns to the queue after a couple of minutes and someone answers twice.',
    domain: 'inbox',
    defer: true,
    ...
    handler: async (ctx) => {
      ...
      return {
        ...
        next: 'Read the thread, write the reply as the attached agent, send it with jeeta.send_message, then call jeeta.complete_reply_job.',
      };
    },
  });
