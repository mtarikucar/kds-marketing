    ...
    safeFetchSpy.mockResolvedValue(
      jsonResponse({
        value: [
          {
            id: 'ext-evt-1',
            subject: 'Optional (free)',
            showAs: 'free',
            start: { dateTime: '2027-06-14T11:00:00.0000000', timeZone: 'UTC' },
            end: { dateTime: '2027-06-14T12:00:00.0000000', timeZone: 'UTC' },
          },
        ],
        '@odata.deltaLink': 'https://graph.microsoft.com/v1.0/me/calendarView/delta?$deltatoken=tok3',
      }),
    );
    prisma.booking.findFirst.mockResolvedValue(null as never); // not ours
    (prisma.booking.deleteMany as jest.Mock).mockResolvedValue({ count: 1 } as never); // had a block from when it was busy
