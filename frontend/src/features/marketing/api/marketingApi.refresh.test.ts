...
// controllable. The instance's interceptors are no-ops here. (This suite
// lives in its OWN file because the sibling marketingApi.test.ts drives the
// REAL registered interceptors and must not see this module-level mock.)
vi.mock('axios', () => {
  const post = vi.fn();
...
import { refreshMarketingToken } from './marketingApi';
