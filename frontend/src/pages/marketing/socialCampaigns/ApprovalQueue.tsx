          ...
          <Card key={it.id}>
            <CardContent className="flex items-center justify-between gap-4 p-4">
              ...
              <div className="flex shrink-0 gap-2">
                <Button size="sm" loading={busy} onClick={() => onReview(it.id, 'approve')}>
                  {t('socialCampaign.approve', 'Approve')}
                </Button>
                <Button size="sm" variant="secondary" disabled={busy} onClick={() => onReview(it.id, 'regenerate')}>
                  {t('socialCampaign.regenerate', 'Regenerate')}
                </Button>
                <Button size="sm" variant="ghost" disabled={busy} onClick={() => onReview(it.id, 'reject')}>
